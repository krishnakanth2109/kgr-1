...
const Home = () => {
  return (
    <div className="bg-[#fbe7ee] min-h-screen">
      {/* Hero Section */}
      <section className="relative bg-gradient-to-r from-pink-500 to-pink-700 text-white shadow-xl rounded-b-3xl overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-28 md:py-36 flex flex-col md:flex-row items-center gap-12">
          ...
          <div className="flex-1 flex flex-col items-center md:items-start text-center md:text-left space-y-8 z-10">
            ...
            <p className="text-lg md:text-xl text-pink-100 max-w-xl font-medium animate-fade-in">
              Shaping the future of students who aspire to build careers in healthcare.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start mt-4">
              <Link
                to="/admissions"
                className="px-8 py-3 bg-white text-pink-600 font-bold rounded-lg shadow-lg 
                hover:bg-gray-100 transition duration-200 hover:scale-105 animate-fade-in"
              >
                Apply Now
              </Link>
              <Link
                ...
                hover:bg-white hover:text-pink-600 transition duration-200 hover:scale-105 animate-fade-in"
              >
                Explore Courses
              </Link>
            </div>
        ...
        {/* Decorative shapes */}
        <div className="absolute top-0 left-0 w-40 h-40 bg-pink-300 rounded-full opacity-30 blur-2xl animate-pulse"></div>
        <div className="absolute bottom-0 right-0 w-56 h-56 bg-pink-400 rounded-full opacity-20 blur-2xl animate-pulse"></div>
      </section>

      {/* Our College Section */}
      <section className="py-24 px-6 max-w-5xl mx-auto text-center space-y-10 animate-fade-in">
        <h2 className="text-4xl md:text-5xl font-extrabold text-pink-800">Our College</h2>
        ...
        <div>
          <Link
            to="/admissions"
            className="inline-block px-10 py-4 bg-pink-600 text-white font-bold rounded-lg shadow-lg 
            hover:bg-pink-700 transition duration-200 hover:scale-105 text-lg"
          >
      ...
      <section className="py-24 px-6 max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-16">
        <div className="flex-1 animate-fade-in">
          <img
            ...
            className="rounded-3xl shadow-2xl border-4 border-pink-600 w-full max-w-md object-cover"
          />
        </div>
        <div className="flex-1 text-center md:text-left space-y-8 animate-fade-in">
          <h2 className="text-4xl md:text-5xl font-extrabold text-pink-800 mb-4">About Us</h2>
          ...
          <Link
            to="/about"
            className="inline-block px-8 py-3 bg-pink-600 text-white font-bold rounded-lg shadow-lg 
            hover:bg-pink-700 transition duration-200 hover:scale-105"
          >
            Learn More
          </Link>
        </div>
